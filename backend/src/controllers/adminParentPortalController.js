...
export const listPortalParents = asyncHandler(async (req, res) => {
  ...
  const parents = await Parent.find(filter)
    ...
    .select('fatherName motherName fatherPhone motherPhone portalEmail isPortalActive isActive lastLoginAt mustChangePassword autoGenerated students')
    .sort('fatherName')
    .lean();
...
export const activatePortal = asyncHandler(async (req, res) => {
  ...
  if (!password) {
    const e = new Error('password is required'); e.status = 400; throw e;
  }
  ...
  if (portalEmail) parent.portalEmail = portalEmail.toLowerCase().trim();
  ...
  ok(res, { message: 'Parent portal activated successfully', data: { isPortalActive: true, studentIds: parent.students } });
});
